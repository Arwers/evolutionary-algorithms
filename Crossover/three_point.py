...
class ThreePointCrossover(CrossoverStrategy):
    def crossover(self, ind_a: Individual, ind_b: Individual) -> tuple[Individual, Individual]:
        ...
        for i in range(len(chromosomes_a)):
            ...
            points = np.sort(np.random.choice(range(1, chr_a.size), size=3, replace=False))
            # Perform the crossover
            new_chr_a = np.concatenate(
                (chr_a[:points[0]], chr_b[points[0]:points[1]], chr_a[points[1]:points[2]], chr_b[points[2]:])
            )
            new_chr_b = np.concatenate(
                (chr_b[:points[0]], chr_a[points[0]:points[1]], chr_b[points[1]:points[2]], chr_a[points[2]:])
            )
